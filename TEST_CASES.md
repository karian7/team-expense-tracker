...
> **안내:** 문서 일부에 등장하는 `/api/monthly-budgets/*` 등 레거시 REST 엔드포인트는 이벤트 소싱 전환으로 제거되었습니다. 최신 환경에서는 `POST /api/events` + `GET /api/events/sync` 조합으로 동일 시나리오를 재현하세요.
...
### TC-002-1: 페이지 로딩 시 월 예산 자동 반영 (Regression)

**목적**: 앱 로딩 시 이번 달에 월 예산이 반영되지 않았으면 자동으로 반영
...
1. `defaultMonthlyBudget` 설정이 0원 이상
2. 이번 달에 "기본 월별 예산" 이벤트가 없음
3. → 자동으로 `BUDGET_IN` 이벤트 생성 (월 1회, 중복 방지)

**사전 조건**:

- 데이터베이스 초기화 (이번 달 예산 이벤트 없음)
- 설정에서 `defaultMonthlyBudget` = 500,000원
...
- ✅ 두 번째 새로고침 시 중복 생성 안됨 (Unique Constraint)
...
### TC-002-3: 중복 조정 내용 방지 (Regression)

**목적**: 같은 달에 동일한 조정 내용으로 중복 등록 방지 및 명확한 에러 메시지 제공

**배경**:

- 버그: Unique Constraint로 중복 방지는 되지만, 에러 메시지가 불명확함
- 수정: Prisma 에러 코드 `P2002` 감지 후 사용자 친화적인 에러 메시지 반환
...
- TC-002-2 완료 (현재 달 예산이 700,000원, "추가 예산 지원" 조정 완료)
...
- ❌ 조정 실패
- ✅ 명확한 에러 메시지 표시:
  - "이미 '추가 예산 지원'라는 내용으로 조정한 기록이 있습니다. 다른 내용으로 입력해주세요."
- ✅ 백엔드에 새 이벤트 생성 안됨 (기존 700,000원 유지)

**해결 방법**:

1. 조정 내용을 다른 텍스트로 변경 (예: "추가 예산 지원 2차")
2. 또는 기존 조정 내용을 수정할 수 없으므로 새로운 설명 사용
...
- [ ] **TC-002-1: 페이지 로딩 시 월 예산 자동 반영**
  - 수정 일자: 2025-12-07
  - 관련 파일: `backend/src/services/settingsService.ts`, `frontend/src/App.tsx`, `frontend/src/hooks/useSettings.ts`
  - 증상: 예산 설정해도 페이지 로딩 시 자동 반영 안됨
