...
export default function SyncStatusIndicator() {
  ...
  const pendingCount = useLiveQuery(() => db.pendingEvents.count(), []);
  ...
  return (
    <div className="relative">
      <button
        ...
        title={
          hasError
            ? `동기화 오류: ${syncStatus?.lastErrorMessage || '알 수 없는 오류'}`
            : `동기화 대기 중: ${pendingCount}건`
        }
      >
        <svg
        ...
        {/* Badge */}
        {hasPending && (
          <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-yellow-500 text-white text-xs font-bold">
